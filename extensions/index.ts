...
export default function (pi: ExtensionAPI) {
	pi.registerTool({
		...
		parameters: Type.Object({
			...
			path: Type.Optional(Type.String({ description: "Limit to file or directory (e.g. src/Service). Git runs from this path, so it can point into a nested repo." })),
		}),
			...
			if (filePath) {
				...
				try {
					...
					if (root) {
						...
						// Deleted dir -> glob its former PHP contents; deleted file -> literal.
